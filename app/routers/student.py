...
def get_all_student(db:Session=Depends(get_db),name:str|None=None,no:str|None=None,current_user:dict=Depends(get_current_user)):
    return student.get_student(db,name,no)

@router.get("/{student_id}",response_model=StudentResponse)
def get_by_id(student_id:int,db:Session=Depends(get_db),current_user:dict=Depends(get_current_user)):
